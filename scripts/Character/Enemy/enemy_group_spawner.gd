...
#int = number of enemies, float = probability of set number to spawn
@export var _enemy_possibilites : Array[float]
...
func _on_enter_room() -> void:
	...
	var number_of_enemies = _get_random_enemy_number()
...
func _get_random_enemy_number() -> int:
	rng.randomize()
	var random_number : float = rng.randf()
	var cumu := 0.0
	for i in range(_enemy_possibilites.size()):
		cumu += _enemy_possibilites[i]
		if random_number < cumu:
			return i

	# Sécurité (ne devrait jamais arriver si les probas = 1)
	return 0

##Check if the enemies probability were entered correctly
func _assert_enemy_possibilities() -> void:
	var assert_proba = 0
	for i in _enemy_possibilites:
		assert_proba += i
	if assert_proba != 1.0:
		push_error("sum of all proba are not equal to 1 (" + String.num(assert_proba) + ") !")
